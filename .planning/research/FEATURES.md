...
| Persisted date range preference per fund | Users choose a range once; finding it reset every page load is friction. | LOW | Write selected range to `fund.config.chart_range` via existing `updateFundConfig` API. Pattern matches existing `chart_bounds` persistence. |
