...
import { fetchHistory, type TimeSeriesPoint, type AllocationData, type HistoryResponse } from '../api/funds'
...
function ChartSkeleton({ height = 'h-[70px] xs:h-[80px] sm:h-[100px] md:h-[120px]' }: { height?: string }) {
  return (
    <div className="bg-slate-800 rounded-lg p-1 xs:p-1.5 sm:p-2 border border-slate-700 relative animate-pulse">
      <div className="h-2 w-20 bg-slate-700 rounded mb-1" />
      <div className={`${height} bg-slate-700/50 rounded`} />
    </div>
  )
}
...
function PieChartSkeleton() {
  return (
    <div className="bg-slate-800 rounded-lg p-1.5 sm:p-2 border border-slate-700 relative animate-pulse snap-start">
        ...
        <div className="flex-1 flex flex-col gap-2 min-w-0">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="flex flex-col gap-0.5">
              ...
              <div className="h-1.5 sm:h-2 bg-slate-700 rounded-sm" />
            </div>
          ))}
        </div>
      </div>
...
// Lazy-loading wrapper that fetches its own data
interface LazyPortfolioChartsProps {
  showTestData: boolean
  filterPlatform: string
}

export function LazyPortfolioCharts({ showTestData, filterPlatform }: LazyPortfolioChartsProps) {
  ...
  useEffect(() => {
    ...
    fetchHistory(showTestData).then(result => {
      if (cancelled) return
      if (result.data) {
    ...
    return (
      <div className="space-y-1.5 xs:space-y-2 sm:space-y-3">
        ...
        <div className="relative">
          ...
          <div className="overflow-x-auto -mx-2 px-2 sm:mx-0 sm:px-0 sm:overflow-x-visible pb-1 sm:pb-0">
  ...
  // No data to display
  if (filteredHistory.currentAllocations.length === 0) {
    return null
  }
