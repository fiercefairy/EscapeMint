        ...
        <div className="bg-slate-800 rounded-lg p-3 border border-slate-700">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-sm flex-wrap gap-y-1.5">
                ...
                <Link to={`/platform/${fund.platform}`} className="text-slate-400 hover:text-white capitalize leading-tight">{fund.platform}</Link>
                <span className="text-slate-600 leading-tight">/</span>
                <span className="text-white font-medium uppercase leading-tight">{fund.ticker}</span>
                {/* Closed Tag */}
                {(fund.config.status === 'closed' || (fund.config.status === undefined && fund.config.fund_size_usd === 0)) && (
                  <span className="px-2 py-1 text-[10px] font-medium leading-none bg-slate-700 text-slate-400 rounded inline-block align-middle">Closed</span>
                )}
                {/* Audited Badge - clickable to toggle */}
                <button
                  onClick={toggleAudited}
                  className={`px-2 py-1 text-[10px] font-medium rounded inline-flex items-center gap-1 leading-none transition-colors align-middle ${
                    fund.config.audited
                ...
                {state?.recommendation && features.allowsRecommendations && (
                  <span
                    className={`px-2 py-1 text-[10px] font-bold leading-none rounded inline-block align-middle ${
                ...
                {state && ((state.cash_available ?? 0) > 0 || (fund.config.margin_enabled && (state.margin_available ?? 0) > 0)) && (
                  <span className="px-2 py-1 text-[10px] font-medium leading-none rounded bg-slate-700/50 text-slate-300 border border-slate-600 inline-block align-middle">
